...
COT_PROMPT = (
    ...
    "Make sure the story reflects the given emotion, incorporates "
    "elements of the art style, and addresses the key elements of "
    "the description provided. Use a narrative format to tell a "
...
class CoTWrapper:
    def __init__(self, baseline_model: FlanT5Model) -> None:
        """
        Initializes the CoTWrapper class.
        :param baseline_model: the base Flan-T5 model for generating stories.
        """
    ...
    def preprocess_dataset(self, dataset: pd.DataFrame) -> pd.DataFrame:
        """
        Tokenizes and prepares the dataset for training and evaluation.

        :param dataset: the dataset to preprocess.
        :return: the tokenized and processed dataset.
        """
        max_length = 512

        def tokenize_function(batch: Dict[str, List[Any]]) -> Dict[str, Any]:
            """
            Tokenizes a batch of data from the dataset.

            :param batch: a batch of data.
            :return: a dictionary containing tokenized inputs and labels.
            """
            prompts = [
                COT_PROMPT.format(
                    emotion=emotion,
    ...
    def clean_generated_text(self, text: str) -> str:
        """
        Cleans and corrects the generated text to improve quality.

        :param text: the generated text.
        :return: the cleaned and corrected text.
        """
    ...
    def generate_with_cot(self, emotion: str, utterance: str, art_style: str,
                          min_tokens: int = 100, max_tokens: int = 200) -> str:
        """
        Generates a story using CoT prompting.

        :param emotion: the emotion to reflect in the story.
        :param utterance: the input description or utterance.
        :param art_style: the art style to incorporate in the story.
        :param min_tokens: minimum number of tokens for the output.
        :param max_tokens: maximum number of tokens for the output.
        :return: the generated story.
        """
        prompt = COT_PROMPT.format(emotion=emotion,
                                   input_description=utterance,
                                   art_style=art_style)
        raw_story = self.baseline_model.generate_story(prompt,
    ...
    def evaluate_with_cot(self, val_dataset: pd.DataFrame) -> dict:
        """
        Evaluates the model on a validation dataset using CoT prompting.
        ...
        def process_example(example):
            try:
                ...
                generated_story = self.generate_with_cot(emotion, utterance,
                                                         art_style)
        ...
        with ThreadPoolExecutor() as executor:
            results = list(filter(None, executor.map(process_example,
                                                     val_dataset)))
        ...
        generated_encodings = self.sbert_model.encode(predictions,
                                                      convert_to_tensor=True)
        reference_encodings = self.sbert_model.encode(references,
                                                      convert_to_tensor=True)
        dissimilarity_scores = (1 - util.cos_sim(
            generated_encodings, reference_encodings).diagonal()).tolist()

        Logger.log_info("Computing the BERT and Dissimilarity scores...")
        bert_score = bertscore.compute(predictions=predictions,
                                       references=references, lang="en")
        average_bert_score = sum(bert_score["f1"]) / len(bert_score["f1"])
        average_dissimilarity = (
            sum(dissimilarity_scores) / len(dissimilarity_scores)
        )
        ...
        return {"BERTScore_F1": bert_score["f1"],
                "Dissimilarity": dissimilarity_scores}

    def plot_metrics(self, bert_scores: list,
                     dissimilarity_scores: list) -> None:
        """
        Plots various evaluation metrics for analysis and visualization.

        :param bert_scores: List of BERTScore F1 values.
        :param dissimilarity_scores: List of dissimilarity scores.
        """
        Logger.log_info("Plotting the results...")
        try:
            # bar plot for bertscore
            plt.figure(figsize=(12, 6))
            plt.bar(range(len(bert_scores)), bert_scores, color="blue",
                    label="BERTScore F1")
            ...
            plt.figure(figsize=(12, 6))
            plt.bar(range(len(dissimilarity_scores)), dissimilarity_scores,
                    color="green", label="Dissimilarity (1 - Similarity)")
            plt.legend()
            ...
            plt.scatter(dissimilarity_scores, bert_scores, alpha=0.7,
                        edgecolor='k')
            ...
            # heatmap
            data = pd.DataFrame({"BERTScore F1": bert_scores,
                                 "Dissimilarity": dissimilarity_scores})
            ...
            sorted_dissimilarity_scores = [
                dissimilarity_scores[i] for i in sorted_indices]

            plt.figure(figsize=(10, 6))
            plt.plot(sorted_bert_scores, label="BERTScore F1", marker='o')
            plt.plot(sorted_dissimilarity_scores, label="Dissimilarity",
                     marker='x')
            ...
            plt.close()
            Logger.log_info("The plots have successfully been generated.")
...
if __name__ == "__main__":
    """
    Main function that uses the CoTWrapper class to train and
    evaluate the model.
    """
    # get the data
    file_path = os.path.join(os.path.dirname(__file__),
                             '../Data/Artemis-cleaned.csv')

    Logger.log_info("Loading dataset...")
    raw_dataset = dl.load_artemis_dataset(file_path)

    Logger.log_info("Splitting the dataset into 70% training and 30% testing.")
    ...
    for i, example in enumerate(val_dataset.select(range(3))):
        ...
        utterance = example["utterance"]
        art_style = example["art_style"]
        generated_story = cot_wrapper.generate_with_cot(emotion, utterance,
                                                        art_style)
        print(f"\nSample {i + 1}:")
